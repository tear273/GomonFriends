...
public class GravityDemo : MonoBehaviour {
	...
	private GravityPlugin _gravityPlugin;
	public Text xText;
	public Text yText;
	public Text zText;
	...
	void Start (){
		...
		_gravityPlugin = GravityPlugin.GetInstance();
		if (_sensorUtilsPlugin.HasGyroscope())
		{
			_gravityPlugin.Init( OnGravity);
			_gravityPlugin.SetDebug(0);
			_gravityPlugin.RegisterSensorListener(SensorDelay.SENSOR_DELAY_NORMAL);
		}else
		{
	...
	private void OnApplicationPause(bool val){
		if(val){
			if(_gravityPlugin!=null){
				_gravityPlugin.RemoveSensorListener();
			}
		}else{
			if(_gravityPlugin!=null){
				_gravityPlugin.RegisterSensorListener(SensorDelay.SENSOR_DELAY_NORMAL);
	...
	private void UpdateSensitivity(int sensitivity){
		if(_gravityPlugin!=null){
			_gravityPlugin.SetSensitivity(sensitivity);
	...
	private void UpdateDelayUpdate(int delayUpdate){
		if(_gravityPlugin!=null){
			_gravityPlugin.SetDelayUpdate(delayUpdate);
	...
	#region Events
	private void OnGravity(string sensorData){
