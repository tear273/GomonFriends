...
public class PedometerPlugin : MonoBehaviour {
	...
	private void Awake(){
		...
		if(Application.platform == RuntimePlatform.Android){
			jo = new AndroidJavaObject("com.gigadrillgames.androidplugin.pedometer.PedometerPlugin");
		}
		#endif
	}
	...
	public void Init(){
		...
		if(Application.platform == RuntimePlatform.Android){
			...
			jo.CallStatic("init",currentActivity,ipedometerCallback);
		}else{
			Message("warning: must run in actual android device");
		}
		#endif
	...
	public void StartPedometerService(SensorDelay sensorType,string serviceNotificationName, string serviceNotificationBodyText){
		int selectedSensorType = (int)sensorType;
		Message("check selected sensor type: " + selectedSensorType);
		#if UNITY_ANDROID
		if(Application.platform == RuntimePlatform.Android){
			jo.CallStatic("startPedometerService",selectedSensorType,serviceNotificationName,serviceNotificationBodyText);
		}else{
			Message("warning: must run in actual android device");
		}
		#endif
