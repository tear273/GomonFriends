...
public class GravityPlugin : MonoBehaviour {
	...
	public static GravityPlugin GetInstance(){
		if(instance==null){
			...
			container.name="GravityPlugin";
			instance = container.AddComponent( typeof(GravityPlugin) ) as GravityPlugin;
	...
	private void Awake(){
		...
		if(Application.platform == RuntimePlatform.Android){
			jo = new AndroidJavaObject("com.gigadrillgames.androidplugin.gravity.GravityPlugin");
		}
		#endif
	}
	...
	public void Init(Action<string> OnGravity){
		...
		if(Application.platform == RuntimePlatform.Android){
			...
			IGravityCallback iGravityCallback = new IGravityCallback();
			iGravityCallback.OnGravity = OnGravity;
			jo.CallStatic("init",currentActivity,iGravityCallback);
		}else{
			Message("warning: must run in actual android device");
		}
		#endif
