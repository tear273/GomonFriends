...
public abstract class UIBasicSprite : UIWidget
{
	...
	[HideInInspector] [SerializeField] protected Color mGradientLeft = Color.white;
	[HideInInspector] [SerializeField] protected Color mGradientRight = new Color(0.7f, 0.7f, 0.7f);
	[HideInInspector] [SerializeField] protected float mRound = 0;
	[HideInInspector] [SerializeField] protected bool mLTRound = true;
	[HideInInspector] [SerializeField] protected bool mRTRound = true;
	[HideInInspector] [SerializeField] protected bool mLBRound = true;
	[HideInInspector] [SerializeField] protected bool mRBRound = true;


	[HideInInspector] [SerializeField] protected bool mApplyLtR = false;
	[HideInInspector] [SerializeField] protected bool mApplyTtB = true;

	[Range(-1f, 1f)]
	[HideInInspector] [SerializeField] protected float mPersentGradientT = 1f;
	[Range(-1f, 1f)]
	[HideInInspector] [SerializeField] protected float mPersentGradientB = 1f;
...
#region Fill Functions
	...
	void AddVertexColours (List<Color> cols, ref Color color, float x, int y,bool lr = false,Color originColor = new Color())
	{
		...
		if (type == Type.Simple || (br.x == 0f && br.y == 0f && br.z == 0f && br.w == 0f))
		{
            if (applyLtR)
            {
                if (lr)
                {
					cols.Add(color * originColor);
				}
				else
                {
					if (x == 0 || x == 2)
					{
						cols.Add(color * mGradientLeft);
					}
					else if (x == 1 || x == 3)
					{
						cols.Add(color * mGradientRight);
					}
				}
	...
	void SimpleFillRound(List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		...
		for (int i = 0; i < slice; i++)
		{
			...
			verts.Add(new Vector3(v.x + (mLBRound ? x1 : 0), v.y + (mLBRound ? y1 : 0)));
			verts.Add(new Vector3(v.x + (mLTRound ? x1 : 0), v.w - (mLTRound ? y1 : 0)));
			...
			else
			{
				...
				if (mApplyLtR)
				{
					AddVertexColours(cols, ref c, 1, 1, true, mGradientRight);
					AddVertexColours(cols, ref c, 1, 2, true, mGradientRight);
					AddVertexColours(cols, ref c, 2, 2, true, mGradientRight);
					AddVertexColours(cols, ref c, 2, 1, true, mGradientRight);
				}
				else
				{
					AddVertexColours(cols, ref c, 1, 1);
		...
		verts.Add(new Vector3((v.z - round) * mPersentGradientT, v.w));
		verts.Add(new Vector3((v.z - round) * mPersentGradientB, v.y));
		uvs.Add(new Vector2(u.x + uvRound, u.y));
		uvs.Add(new Vector2(u.x + uvRound, u.w));
		uvs.Add(new Vector2((u.z - uvRound) * mPersentGradientT, u.w));
		uvs.Add(new Vector2((u.z - uvRound) * mPersentGradientB, u.y));
		if (!mApplyGradient)
		{
			cols.Add(c);
			cols.Add(c);
		...
		for (int i = 0; i < slice; i++)
		{
			...
			verts.Add(new Vector3(v.z - (mRBRound ? x1 : 0), v.y + (mRBRound ? y1 : 0)));
			verts.Add(new Vector3(v.z - (mRTRound ? x1 : 0), v.w - (mRTRound ? y1 : 0)));
			verts.Add(new Vector3((v.z - x2) * mPersentGradientT, v.w - y2));
			verts.Add(new Vector3((v.z - x2) * mPersentGradientB, v.y + y2));
			uvs.Add(new Vector2(u.z - uvX1, u.y + uvY1));
			uvs.Add(new Vector2(u.z - uvX1, u.w - uvY1));
			uvs.Add(new Vector2((u.z - uvX2) * mPersentGradientT, u.w - uvY2));
			uvs.Add(new Vector2((u.z - uvX2) * mPersentGradientB, u.y + uvY2));
			if (!mApplyGradient)
			{
				cols.Add(c);
				cols.Add(c);
			...
			else
			{
                if (mApplyLtR)
                {
					AddVertexColours(cols, ref c, 1, 1,true,mGradientLeft);
					AddVertexColours(cols, ref c, 1, 2, true, mGradientLeft);
					AddVertexColours(cols, ref c, 2, 2, true, mGradientLeft);
					AddVertexColours(cols, ref c, 2, 1, true, mGradientLeft);
				}
                else
                {
					AddVertexColours(cols, ref c, 1, 1);
